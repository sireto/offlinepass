...
const Button: React.FC<ButtonProps> = ({ children, className, isLoading, disabled, fullWidth, shape = 'square', color = 'primary', variant = 'solid', size = 'medium', onClick, ...buttonProps }) => {
    ...
    return (
        <button onClick={onClick} className={cn('flex items-center justify-center', fullWidth && 'w-full', buttonColorClassNames, buttonDripColor, sizeClassNames[0], shapeClassNames, className)} {...buttonProps}>
            {children}
        </button>
    );
};
